...
const mockInterviewRequests = [
  ...
  { id: 3, date: '2024-03-20', time: '16:00', questionnaire: 'Bien-être au travail', status: 'confirmed' },
  { id: 4, date: '2024-02-10', time: '11:00', questionnaire: 'Évaluation des compétences', status: 'declined' },
  { id: 5, date: '2024-02-05', time: '09:00', questionnaire: 'Feedback sur le management', status: 'pending' },
]
...
export default function InterviewRequests() {
  ...
  const handleArchive = (id: number) => {
    ...
    if (requestToArchive) {
      ...
      setArchivedRequests([...archivedRequests, { ...requestToArchive, archived: true }])
  ...
  const RequestCard = ({ request, showArchive = false }) => (
    <Card className="mb-4">
        ...
        <div className="flex items-center space-x-2">
          {request.status === 'pending' && (
            <>
              ...
              <Button onClick={() => handleDecline(request.id)} className="bg-[#e74c3c] hover:bg-[#c0392b] text-white">
          ...
          {showArchive && (request.status === 'confirmed' || request.status === 'declined') && (
            <Button onClick={() => handleArchive(request.id)} variant="outline" className="border-[#3c6382] text-[#3c6382] hover:bg-[#3c6382]/10">
            ...
            <Tabs defaultValue="pending" className="w-full">
              ...
              <TabsContent value="confirmed">
                {requests.filter(req => req.status === 'confirmed').map(request => (
                  <RequestCard key={request.id} request={request} showArchive={true} />
                ))}
              </TabsContent>
